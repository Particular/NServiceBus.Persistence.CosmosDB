...
public class When_custom_container_is_overwritten : NServiceBusAcceptanceTest
{
    ...
    public async Task Should_persist_into_the_provided_container()
    {
        ...
        ItemResponse<dynamic> orderProcessSagaData = await SetupFixture.Container.ReadItemAsync<dynamic>(context.OrderProcessId,
            new PartitionKey(context.OrderProcessId));
        ItemResponse<dynamic> shippingProcessSagaData = await nonDefaultContainer.ReadItemAsync<dynamic>(context.ShippingProcessId,
    ...
    public class EndpointWithFluentExtractor : EndpointConfigurationBuilder
    {
        public EndpointWithFluentExtractor() =>
            EndpointSetup<DefaultServer>((config, r) =>
            {
                ...
                {
                    if (headers.TryGetValue(Headers.SagaType, out string sagaTypeHeader) && sagaTypeHeader.Contains(nameof(ShipOrderSaga)))
                    {
            ...
            IHandleMessages<CompleteOrder>
        {
            ...
            public Task Handle(StartOrder message, IMessageHandlerContext context)
            {
                ...
                var shipOrder = new ShipOrder { OrderId = message.OrderId };
