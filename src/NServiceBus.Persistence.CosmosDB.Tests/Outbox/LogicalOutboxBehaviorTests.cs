...
public class LogicalOutboxBehaviorTests
{
    ...
    public async Task Should_clear_added_pending_operations_and_restore_ones_from_outbox_record()
    {
        ...
        var behavior = new LogicalOutboxBehavior(containerHolderHolderResolver, new JsonSerializer(), new ExtractorConfigurationHolder(), true);
...
{
    public override Task<ResponseMessage> ReadItemStreamAsync(string id, PartitionKey partitionKey, ItemRequestOptions requestOptions = null, CancellationToken cancellationToken = new())
    {
        var responseMessage = new ResponseMessage(HttpStatusCode.OK) { Content = new MemoryStream(Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(ReadItemStreamOutboxRecord(id, partitionKey)))) };
