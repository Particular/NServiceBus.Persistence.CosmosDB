...
public class OutboxPersisterTests
{
    ...
    public async Task Should_return_null_when_not_found()
    {
        ...
        var persister = new OutboxPersister(containerHolderHolderResolver, JsonSerializer.Create(), "SomeProcessingEndpoint", true, new ExtractorConfiguration { HasCustomHeaderExtractors = false, HasCustomMessageExtractors = false }, 0);
