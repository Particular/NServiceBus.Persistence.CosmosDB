...
public class StorageSessionTests
{
    ...
    public async Task Should_complete_when_outbox_transaction_completes()
    {
        ...
        bool firstOperationWasDisposed = firstOperation.WasDisposed;
        bool firstOperationWasApplied = firstOperation.WasApplied;
        bool secondOperationWasDisposed = secondOperation.WasDisposed;
        bool secondOperationWasApplied = secondOperation.WasApplied;
    ...
    public async Task Should_throw_on_complete_when_no_container_available()
    {
        ...
        Exception exception = Assert.ThrowsAsync<Exception>(async () => await storageSession.CompleteAsync());
    ...
    public async Task Should_throw_on_outbox_transaction_commit_when_no_container_available()
    {
        ...
        Exception exception = Assert.ThrowsAsync<Exception>(async () => await outboxTransaction.Commit());
        Assert.That(exception.Message, Is.EqualTo("Unable to retrieve the container name and the partition key during processing. Make sure that either `persistence.Container()` is used or the relevant container information is available on the message handling pipeline."));
    }
    ...
    public async Task Should_execute_operations_with_same_partition_key_together()
    {
        ...
        var firstOperation = new FakeOperation { PartitionKey = new PartitionKey("PartitionKey1") };
        storageSession.AddOperation(firstOperation);
        var secondOperation = new FakeOperation { PartitionKey = new PartitionKey("PartitionKey1") };
        storageSession.AddOperation(secondOperation);
    ...
    public async Task Should_execute_with_outbox_transaction_operations_with_same_partition_key_together()
    {
        ...
        bool firstOperationWasApplied = firstOperation.WasApplied;
        bool secondOperationWasApplied = secondOperation.WasApplied;
        TransactionalBatch firstOperationAppliedBatch = firstOperation.AppliedBatch;
        TransactionalBatch secondOperationAppliedBatch = secondOperation.AppliedBatch;
    ...
    public async Task Should_execute_operations_with_different_partition_key_distinct()
    {
        ...
        var firstOperation = new FakeOperation { PartitionKey = new PartitionKey("PartitionKey1") };
        storageSession.AddOperation(firstOperation);
        var secondOperation = new FakeOperation { PartitionKey = new PartitionKey("PartitionKey2") };
        storageSession.AddOperation(secondOperation);
    ...
    public async Task Should_dispose_release_operations_when_operations_successful()
    {
        ...
        var operation = new FakeOperation { PartitionKey = new PartitionKey("PartitionKey1") };
        storageSession.AddOperation(operation);
        var releaseOperation = new ReleaseLockOperation { PartitionKey = new PartitionKey("PartitionKey1") };
    ...
    public async Task Should_dispose_release_operations_on_outbox_transaction_commit_when_operations_successful()
    {
        ...
        bool operationWasApplied = operation.WasApplied;
        bool operationWasDisposed = operation.WasDisposed;
        bool releaseOperationWasApplied = releaseOperation.WasApplied;
        bool releaseOperationWasDisposed = releaseOperation.WasDisposed;
    ...
    public async Task Should_execute_and_dispose_release_operations_with_same_partition_key_together_when_not_completed()
    {
        ...
        var firstReleaseOperation = new ReleaseLockOperation { PartitionKey = new PartitionKey("PartitionKey1") };
        storageSession.AddOperation(firstReleaseOperation);

        var secondReleaseOperation = new ReleaseLockOperation { PartitionKey = new PartitionKey("PartitionKey1") };
    ...
    public async Task Should_execute_and_dispose_release_operations_on_outbox_transaction_dispose_with_same_partition_key_together_when_not_completed()
    {
        ...
        bool firstReleaseOperationWasApplied = firstReleaseOperation.WasApplied;
        bool firstReleaseOperationWasDisposed = firstReleaseOperation.WasDisposed;
        TransactionalBatch firstReleaseOperationAppliedBatch = firstReleaseOperation.AppliedBatch;
        bool secondReleaseOperationWasApplied = secondReleaseOperation.WasApplied;
        bool secondReleaseOperationWasDisposed = secondReleaseOperation.WasDisposed;
        TransactionalBatch secondReleaseOperationAppliedBatch = secondReleaseOperation.AppliedBatch;
    ...
    public async Task Should_execute_and_dispose_release_operations_as_best_effort()
    {
        ...
        var secondReleaseOperation = new ReleaseLockOperation { PartitionKey = new PartitionKey("PartitionKey2") };
        storageSession.AddOperation(secondReleaseOperation);
    ...
    public async Task Should_not_dispose_release_operations_when_operations_not_successful()
    {
        ...
        var operation = new ThrowOnApplyOperation { PartitionKey = new PartitionKey("PartitionKey1") };
        storageSession.AddOperation(operation);
        var releaseOperation = new ReleaseLockOperation { PartitionKey = new PartitionKey("PartitionKey1") };
        storageSession.AddOperation(releaseOperation);
    ...
    class FakeContainer : Container
    {
        ...
        public override Task<int?> ReadThroughputAsync(CancellationToken cancellationToken = new()) => throw new NotImplementedException();

        public override Task<ThroughputResponse> ReadThroughputAsync(RequestOptions requestOptions, CancellationToken cancellationToken = new()) => throw new NotImplementedException();
        ...
        public override Task<ResponseMessage> ReplaceItemStreamAsync(Stream streamPayload, string id, PartitionKey partitionKey,
            ItemRequestOptions requestOptions = null, CancellationToken cancellationToken = new()) =>
            throw new NotImplementedException();
        ...
        public override Task<ItemResponse<T>> PatchItemAsync<T>(string id, PartitionKey partitionKey, IReadOnlyList<PatchOperation> patchOperations,
            PatchItemRequestOptions requestOptions = null, CancellationToken cancellationToken = new()) =>
            throw new NotImplementedException();
        ...
        public override Task<IReadOnlyList<FeedRange>> GetFeedRangesAsync(CancellationToken cancellationToken = new()) => throw new NotImplementedException();
