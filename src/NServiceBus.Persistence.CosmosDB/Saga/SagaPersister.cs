namespace NServiceBus.Persistence.CosmosDB
{
    ...
    class SagaPersister : ISagaPersister
    {
        ...
        public async Task<TSagaData> Get<TSagaData>(Guid sagaId, SynchronizedStorageSession session, ContextBag context) where TSagaData : class, IContainSagaData
        {
            ...
            {
                (sagaNotFound, sagaData) = await ReadSagaData<TSagaData>(sagaId, context, container, partitionKey, CancellationToken.None).ConfigureAwait(false);
                ...
                if (sagaNotFound && migrationModeEnabled &&
                    await FindSagaIdInMigrationMode(sagaId, context, container, CancellationToken.None).ConfigureAwait(false) is { } migratedSagaId)
                {
                    partitionKey = GetPartitionKey(context, migratedSagaId);
                    (_, sagaData) = await ReadSagaData<TSagaData>(migratedSagaId, context, container, partitionKey, CancellationToken.None).ConfigureAwait(false);
                }
                return sagaData;
            }

            (sagaNotFound, sagaData) = await AcquireLease<TSagaData>(sagaId, context, container, partitionKey, CancellationToken.None).ConfigureAwait(false);
            // if the previous lookup by id wasn't successful and the migration mode is enabled try to query for the saga data because the saga id probably represents
            // the saga id of the migrated saga.
            if (sagaNotFound && migrationModeEnabled &&
                await FindSagaIdInMigrationMode(sagaId, context, container, CancellationToken.None).ConfigureAwait(false) is { } previousSagaId)
            {
                partitionKey = GetPartitionKey(context, previousSagaId);
                (_, sagaData) = await AcquireLease<TSagaData>(previousSagaId, context, container, partitionKey, CancellationToken.None).ConfigureAwait(false);
            }
            ...
            where TSagaData : class, IContainSagaData
        {
            ...
            if (!pessimisticLockingEnabled)
            {
                (_, sagaData) = await ReadSagaData<TSagaData>(sagaId, context, container, partitionKey, CancellationToken.None).ConfigureAwait(false);
                return sagaData;
            }

            (_, sagaData) = await AcquireLease<TSagaData>(sagaId, context, container, partitionKey, CancellationToken.None).ConfigureAwait(false);
            storageSession.AddOperation(new SagaReleaseLock(sagaData, partitionKey, serializer, context));
            return sagaData;
        }
